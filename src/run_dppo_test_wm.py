...
if __name__ == "__main__":
    ...
    # ------------ 以上�? TC-Bot 自带的参�?------------
    parser.add_argument('--episodes', dest='episodes', default=100, type=int,
    ...
    parser.add_argument('--early_stop_world_model', default=True, type=bool,
                        help='True/False')
    parser.add_argument('--world_model_net_type', default='attention_r', type=str,
    ...
    if agt == 9 and params['trained_model_path'] == None:
        ...
        for episode in tqdm(xrange(episodes), desc=params['write_model_dir'].split('/')[-1],
            ...
            simulation_res = simulation_epoch(params['per_episode_time_steps'],
                                              ...
                                              # not stop_world_model,
                                              use_user=True)
            agent.train()
            _simulation_res = {}
            _simulation_res['success_rate'] = 0.
            world_model.dump_memory_dataset(params['write_model_dir'],
                                            'wm_memory_{}.pt'.format(episode))
            world_model.load_memory_dataset(params['write_model_dir'],
                                            'wm_memory_{}.pt'.format(episode))
...
def test_wm():
    ...
    for episode in tqdm(range(100)):
        print '\n\n'
        # 读取数据，训练
        dataset = world_model.load_memory_dataset(params['write_model_dir'],
                                                  'wm_memory_{}.pt'.format(episode))
        acc_reward, acc_term, acc_user_action = world_model.train(epochs=100, dataset=dataset)
        # DataFrame 中添加新行
        wm_train_res = wm_train_res.append(
            {'epoch': episode, 'acc_reward': acc_reward, 'acc_term': acc_term,
             'acc_user_action': acc_user_action}, ignore_index=True)
    ...
    # 保存DataFrame到 .csv 文件
    dir_name = os.path.join(params['write_model_dir'], 'attention_on_r_a_epoch_100', 'run1')
