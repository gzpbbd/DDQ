...
class AgentDQN(Agent):
    ...
    def train(self, batch_size=1, num_batches=100):
        ...
        self.running_expereince_pool = list(self.experience_replay_pool)[-420:] + list(
            self.experience_replay_pool_from_model)[-1680:]

        all_pool_size = len(self.experience_replay_pool) + len(
            self.experience_replay_pool_from_model)

        logging.debug("len(running_expereince_pool) {}, all_pool_size {}".format(len(
            self.running_expereince_pool), all_pool_size))
        for iter_batch in range(num_batches):
            for iter in range(all_pool_size / (batch_size)):
